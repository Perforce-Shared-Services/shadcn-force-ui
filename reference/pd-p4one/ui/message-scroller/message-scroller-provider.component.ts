...
/** Safety net if the browser never fires `scrollend` (matches the registry button's own 400ms exit duration). */
...
/**
 * Angular port of @force-ui/message-scroller's `MessageScrollerProvider` +
 * its three hooks (`useMessageScroller`, `useMessageScrollerScrollable`,
 * `useMessageScrollerVisibility`).
 *
 * Hand-ported behavior (documented parity gap, maintainer-approved
 * 2026-08-18): the registry component has no `@radix-ng/primitives`
 * equivalent — it wraps `@shadcn/react`'s `MessageScrollerPrimitive`, a
 * headless auto-scroll-to-bottom chat behavior library with no Angular- or
 * web-component-native port available (checked: `shcnwc/shadcn-web-components`
 * has no chat/scroll component). Reimplemented natively rather than adding
 * the React dependency, same "hand-port vs. bespoke JS" call already made for
 * `scroll-area`, `input-otp`, `stepper`, and `calendar`.
 *
 * Rather than exposing three separate hook functions, every descendant reads
 * this one injected instance — Angular's DI naturally collapses
 * `useMessageScroller()` (scrollToMessage/scrollToEnd/scrollToStart),
 * `useMessageScrollerScrollable()` (canScrollToStart/canScrollToEnd), and
 * `useMessageScrollerVisibility()` (currentAnchorId/visibleMessageIds) into
 * one context object.
 */
@Component({
  ...
  providers: [
    { provide: MESSAGE_SCROLLER_CONTEXT, useExisting: forwardRef(() => MessageScrollerProviderComponent) },
  ],
})
export class MessageScrollerProviderComponent {
  ...
  readonly defaultScrollPosition = input<MessageScrollerScrollPosition>(MessageScrollerScrollPosition.End);
   ...
   * settled. Always finishes with a forced `updateEdgeState()` — the
   * `scrollTo` calls above only trigger a real native `scroll` event (which
   * is what normally refreshes `isAtStart`/`isAtEnd`) when they actually
   * move the viewport; scrolling to `start` when already at `scrollTop: 0`
   * is a no-op, which would otherwise leave the edge signals at their
   * pre-layout default.
   */
  ...
  scrollToEnd(smooth = true): void {
    this.runAutoScroll((el) => el.scrollTo({ top: el.scrollHeight, behavior: smooth ? 'smooth' : 'auto' }));
  }
  ...
  scrollToMessage(messageId: string, smooth = true): void {
    const target = this.contentEl?.querySelector<HTMLElement>(`[data-message-id="${messageId}"]`);
    if (!target) {
      return;
    }
  ...
  private ensureVisibilityObserver(): IntersectionObserver {
    if (!this.visibilityObserver) {
      this.visibilityObserver = new IntersectionObserver((entries) => this.onIntersect(entries), {
  ...
  private updateEdgeState(): void {
    ...
    this.isAtEndSignal.set(el.scrollHeight - el.scrollTop - el.clientHeight <= AT_EDGE_THRESHOLD_PX);
  }
}
