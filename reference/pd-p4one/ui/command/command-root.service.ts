 ...
 * `CommandGroup` visibility. Keyboard navigation (`move`, `first`, `last`) and
 * activation (`selectActive`) are driven from `CommandInput` / the root's
 * keydown handler.
 */
@Injectable()
export class CommandRootService {
  ...
  private filterFn: (value: string, search: string, keywords?: string[]) => number = commandScore;

  constructor() {
    // Re-anchor the highlight to the top match whenever the visible set changes
    // — a search edit OR items registering/updating (so an async result injected
    // into an active search still gets highlighted). Tracks `visibleIds()` (which
    // depends on search + items + scores) and writes `activeId` `untracked`; the
    // effect never reads `activeId`, so manual keyboard / pointer navigation is
    // NOT reset (the previous syncActive-in-effect design read activeId and
    // fought every move, pinning the highlight to the top).
    effect(() => {
      this.visibleIds();
   ...
   * combobox input binds to `aria-activedescendant`. Guards against pointing AT
   * to a `[hidden]` option during register/unregister churn (WCAG 4.1.2).
   */
   ...
   * while no search is active (the full list is shown on open — nothing to
   * announce) AND when there are zero matches — in the no-results case
   * `CommandEmpty` is the live region that speaks its own (caller-written) text,
   * so this stays silent to avoid announcing two different strings for one state.
   */
