...
import { Calendar17Component } from './calendar-17.component';

/**
 * `calendar-17` — with time picker inline. Composition of the already-ported
 * `ui/calendar`, `ui/card`, `ui/input`, and `ui/label` primitives; no new
 * tokens or variants are introduced by this block.
 */
const meta: Meta<Calendar17Component> = {
  title: 'Blocks/calendar/calendar-17',
  ...
  decorators: [
    moduleMetadata({
      imports: [Calendar17Component, Calendar, Card, CardContent, CardFooter, Input, Label],
    }),
  ],
  render: () => ({
    template: `
      <div class="flex min-h-[32rem] w-full items-center justify-center bg-background p-6">
        <app-block-calendar-17 />
      </div>
    `,
  }),
};
...
export default meta;
type Story = StoryObj<Calendar17Component>;

/** The full calendar-17 composition: a card wrapping a calendar plus an inline Start-End time row. */
