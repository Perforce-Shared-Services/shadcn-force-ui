/**
 * Signup 05 block — SINGLE SWAP POINT for the block's own decorative glyph.
 *
 * Same convention as `button.icons.ts` / `login-05.icons.ts`: raw inline SVG,
 ...
 * Resources" library, standalone-mark node), fill swapped to `currentColor`
 * so it inherits the block's text color like every other icon in this app.
 * Same markup as `login-05.icons.ts` — both blocks use the identical
 * icon-only mark above a centered "Welcome to Perforce" heading, no card.
 *
...
export const SIGNUP_05_LOGO_SVG = PERFORCE_MARK_SVG;
