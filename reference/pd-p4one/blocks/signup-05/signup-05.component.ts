...
import { SIGNUP_05_LOGO_SVG } from './signup-05.icons';

/**
 * Angular port of the Force UI Block `signup-05` ("A simple signup form with
 * social providers").
 *
 ...
 * Structurally identical to `login-05` (no card, centered brand mark +
 * heading, single email field, primary submit, "Or" separator, two OAuth
 * buttons, terms footnote) — the two blocks share the same layout family in
 * the upstream registry, so this port reuses that established shape.
 *
 * Copy deviation: the Figma frame (fileKey `jr1JErMIXt6T2BakbG2iBI`, node
 * `18751:258819`) puts a "Forgot your password?" link on the Email field's
 * label row — a copy-paste artifact from a login-block template, since a
 * signup form has no password yet to forget (same template-bleed family as
 * the `signup-02` footer-copy deviation documented there). Intentionally
 * omitted here rather than reproduced; the upstream registry source for
 * `signup-05` doesn't carry it either.
 *
 * The Figma frame's icon-only brand mark is a generic placeholder glyph, not
 * the real Perforce mark. As with `login-05`, this port renders the actual
 * Perforce ribbon mark via the `signup-05.icons.ts` swap-point instead of
 * reproducing the placeholder.
 *
      ...
      <div class="flex w-full max-w-sm flex-col gap-6">
        <form (submit)="onSubmit($event)">
          <div uiFieldGroup>
            ...
            <div uiField>
              <label uiFieldLabel for="signup-05-email">Email</label>
              <input
                uiInput
                id="signup-05-email"
                type="email"
...
export class SignupBlock05Component {
  /**
   * Sanitizer-trusted inline SVG — the static Perforce brand-mark markup
   * defined in `signup-05.icons.ts`, same swap-point pattern as every other
  ...
  protected onSubmit(event: Event): void {
    ...
    console.log('[signup-05] submit');
  }

  protected onContinueWithApple(): void {
    console.log('[signup-05] continue with Apple');
  }

  protected onContinueWithGoogle(): void {
    console.log('[signup-05] continue with Google');
  }
}
