/**
 * OTP 05 block — SINGLE SWAP POINT for the block's own decorative glyph.
 *
 * Same convention as `button.icons.ts` / `login-05.icons.ts` / `signup-05.icons.ts`:
 * raw inline SVG, injected as trusted HTML by the component. The upstream
 * registry source (`GalleryVerticalEnd` from `lucide-react`) is a generic
 * placeholder glyph — this app replaces it with the real Perforce brand mark
 * (Figma "The Force - Resources" library, node 1:1253, "Theme=Brand,
 * Style=Symbol"), fill swapped to `currentColor` so it inherits the block's
 * text color like every other icon in this app. Same markup as
 * `login-05.icons.ts` / `signup-05.icons.ts` — all three blocks share the
 * identical icon-only mark above a centered heading, no card.
 */
...
/** Decorative brand-mark glyph shown above the "Enter verification code" title. */
export const OTP_05_LOGO_SVG = PERFORCE_MARK_SVG;
