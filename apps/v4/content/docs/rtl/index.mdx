---
title: "RTL"
description: "Right-to-left support for Force UI components."
---

Force UI components have first-class support for right-to-left (RTL) layouts. Text alignment, positioning, and directional styles automatically adapt for languages like Arabic, Hebrew, and Persian.
...
Automatic RTL transformation via the CLI is only available for projects created using `shadcn create` with the new styles (`base-force-ui, radix-force-ui, etc.).
...
```bash
npx shadcn@latest add @force-ui/direction
```
