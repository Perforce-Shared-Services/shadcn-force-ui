---
title: Monorepo
description: Using Force UI components and CLI in a monorepo.
---

Until now, using Force UI in a monorepo was a bit of a pain. You could add
...
For example, if you run `npx shadcn@latest add @force-ui/button`, the CLI will install the button component under `packages/ui` and update the import path for components in `apps/web`.

If you run `npx shadcn@latest add @force-ui/login-01`, the CLI will install the `button`, `label`, `input` and `card` components under `packages/ui` and the `login-form` component under `apps/web/components`.
