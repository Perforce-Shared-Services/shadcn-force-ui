...
```bash
npx shadcn@latest add @force-ui/resizable
```
...
<ComponentPreview styleName="radix-force-ui" name="resizable-vertical" />

## Handle

Use the `withHandle` prop on `ResizableHandle` to show a visible handle.

<ComponentPreview styleName="radix-force-ui" name="resizable-handle" />
...
<Callout>
  The Force UI wrapper components (`ResizablePanelGroup`, `ResizablePanel`,
