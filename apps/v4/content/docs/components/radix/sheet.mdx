...
<ComponentPreview styleName="radix-force-ui" name="sheet-demo" />
...
```bash
npx shadcn@latest add @force-ui/sheet
```
...
<ComponentPreview styleName="radix-force-ui" name="sheet-side" />

## No Close Button

Use `showCloseButton={false}` on `SheetContent` to hide the close button.

<ComponentPreview styleName="radix-force-ui" name="sheet-no-close-button" />

## RTL

To enable RTL support in Force UI, see the [RTL configuration guide](/docs/rtl).

<ComponentPreview styleName="radix-force-ui" name="sheet-rtl" direction="rtl" />
